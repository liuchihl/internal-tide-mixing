...
## 1) whole domain
    for n in 1:length(t)
        ∇κ∇B = ds_3D["∇κ∇B"][:,:,:,n:n];   # ∇⋅κ∇B: buoyancy flux divergence
...
## 2) canyon
    ...
    for n in 1:length(t)
        ∇κ∇B = ds_3D["∇κ∇B"][:,south:north,:,n:n];   # ∇⋅κ∇B: buoyancy flux divergence
        div_uB = ds_3D["div_uB"][:,south:north,:,n:n];
...
for n in 1:length(t)
    ∇κ∇B = ds_3D["∇κ∇B"][:,vcat(1:south,north:end),:,n:n];   # ∇⋅κ∇B: buoyancy flux divergence
    div_uB = ds_3D["div_uB"][:,vcat(1:south,north:end),:,n:n];
