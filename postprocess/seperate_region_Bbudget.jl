...
for tᶠ in [453.0]#:1.0:462.0
    @info "Processing tᶠ = $tᶠ ..."
    θ = 3.6e-3
    if θ == 3.6e-3
        simname = "tilt"
    else
    ...
    dB̄dy = nothing
    Bz_center = nothing
    Bx_center_manual = nothing
    By_center_manual = nothing
    u_bar_∇B_bar_manual = nothing
    u_prime∇B_prime = nothing
    mask_sill = nothing
    mask_flanks = nothing
    mask_rest = nothing
...
#### plot the buoyancy budget terms at different regions, but first, we need to take the average over 10 TP of the 1 TP avg data
using PyPlot
using NCDatasets
simname = "tilt"  # or "flat" depending on the simulation
count = 0
# preallocate arrays for the budget terms
dBdt_rest = zeros(length(bin_center), 1)
∇κ∇B_rest = zeros(length(bin_center), 1)
div_uB_rest = zeros(length(bin_center), 1)
u_bar_∇B_bar_rest = zeros(length(bin_center), 1)
u_prime∇B_prime_rest = zeros(length(bin_center), 1)
dBdt_flanks = zeros(length(bin_center), 1)
∇κ∇B_flanks = zeros(length(bin_center), 1)
div_uB_flanks = zeros(length(bin_center), 1)
u_bar_∇B_bar_flanks = zeros(length(bin_center), 1)
u_prime∇B_prime_flanks = zeros(length(bin_center), 1)
dBdt_sill = zeros(length(bin_center), 1)
∇κ∇B_sill = zeros(length(bin_center), 1)
div_uB_sill = zeros(length(bin_center), 1)
u_bar_∇B_bar_sill = zeros(length(bin_center), 1)
u_prime∇B_prime_sill = zeros(length(bin_center), 1)
mask_sill = zeros(Nx, Ny)
mask_flanks = zeros(Nx, Ny)
mask_rest = zeros(Nx, Ny)
bin_center_saved = zeros(length(bin_center), 1)

for tᶠ = 453.0:1.0:462.0
    # Load the saved data
    @info "Processing tᶠ = $tᶠ ..."
    ...
    # we only need to load the masks once
    if tᶠ == 453.0
...
# Average over the number of time points
dBdt_rest ./= count
∇κ∇B_rest ./= count
div_uB_rest ./= count
u_bar_∇B_bar_rest ./= count
u_prime∇B_prime_rest ./= count  
dBdt_flanks ./= count
∇κ∇B_flanks ./= count
div_uB_flanks ./= count
u_bar_∇B_bar_flanks ./= count
u_prime∇B_prime_flanks ./= count  
dBdt_sill ./= count
∇κ∇B_sill ./= count
div_uB_sill ./= count
u_bar_∇B_bar_sill ./= count
u_prime∇B_prime_sill ./= count

# calculate the derivatives of the budget terms with respect to hab
dBdt_rest_deriv = diff(dBdt_rest[:,1])./ diff(bin_center_saved)
∇κ∇B_rest_deriv = diff(∇κ∇B_rest[:,1])./ diff(bin_center_saved)
div_uB_rest_deriv = diff(div_uB_rest[:,1])./ diff(bin_center_saved)
u_bar_∇B_bar_rest_deriv = diff(u_bar_∇B_bar_rest[:,1])./ diff(bin_center_saved)
u_prime∇B_prime_rest_deriv = diff(u_prime∇B_prime_rest[:,1])./ diff(bin_center_saved)
dBdt_flanks_deriv = diff(dBdt_flanks[:,1])./ diff(bin_center_saved)
∇κ∇B_flanks_deriv = diff(∇κ∇B_flanks[:,1])./ diff(bin_center_saved)
div_uB_flanks_deriv = diff(div_uB_flanks[:,1])./ diff(bin_center_saved)
u_bar_∇B_bar_flanks_deriv = diff(u_bar_∇B_bar_flanks[:,1])./ diff(bin_center_saved)
u_prime∇B_prime_flanks_deriv = diff(u_prime∇B_prime_flanks[:,1])./ diff(bin_center_saved)
dBdt_sill_deriv = diff(dBdt_sill[:,1])./ diff(bin_center_saved)
∇κ∇B_sill_deriv = diff(∇κ∇B_sill[:,1])./ diff(bin_center_saved)
div_uB_sill_deriv = diff(div_uB_sill[:,1])./ diff(bin_center_saved)
u_bar_∇B_bar_sill_deriv = diff(u_bar_∇B_bar_sill[:,1])./ diff(bin_center_saved)
u_prime∇B_prime_sill_deriv = diff(u_prime∇B_prime_sill[:,1])./ diff(bin_center_saved)
...
tᶠ = 453.0:1.0:462.0  # Time points for averaging
...
# Column 1: Topography map (spans all rows)
ax_map = subplot2grid((3, 3), (0, 0), rowspan=3, colspan=1)

# Column 2: Budget plots (full y-range)
ax_sill = subplot2grid((3, 3), (0, 1), rowspan=1, colspan=1)
ax_flanks = subplot2grid((3, 3), (1, 1), rowspan=1, colspan=1)
ax_rest = subplot2grid((3, 3), (2, 1), rowspan=1, colspan=1)

# Column 3: Zoomed-in budget plots (y=0:100)
ax_sill_zoom = subplot2grid((3, 3), (0, 2), rowspan=1, colspan=1)
ax_flanks_zoom = subplot2grid((3, 3), (1, 2), rowspan=1, colspan=1)
ax_rest_zoom = subplot2grid((3, 3), (2, 2), rowspan=1, colspan=1)
...
ax_map.contour(
    ...
    levels=[1.5, 2.5], colors="white", linewidths=1.5
)
# Add bathymetry contours
contours = ax_map.contour(
    xC / 1e3, yC / 1e3, z_interp', 
    0:200:2200, colors="black", alpha=0.7, linewidths=0.8
)
...
function plot_budget_region(ax, dBdt, ∇κ∇B, div_uB, u_bar_∇B_bar, u_prime∇B_prime, bin_center, title, region_color)
    ...
    ax.plot(-div_uB*1e9, bin_center, color=budget_colors[3], linewidth=2.5, linestyle="--", label=budget_labels[3])
...
# ax_rest.set_xlim(-0.8,1.2)
ax_rest.set_xlabel(L"[10^{-9} \, \text{m s}^{-3}]", fontsize=14)
# ax_rest.legend(loc="upper right", fontsize=12, frameon=true, framealpha=0.93)

# --- Zoomed-in budget plots (y=0:100) ---
plot_budget_region(ax_sill_zoom, dBdt_sill, ∇κ∇B_sill, div_uB_sill, u_bar_∇B_bar_sill, 
                  u_prime∇B_prime_sill, bin_center_saved, "Sill (Zoom)", region_colors[1])
ax_sill_zoom.set_ylim(0, 100)
# ax_sill_zoom.set_xlim(-0.8,1.2)
plot_budget_region(ax_flanks_zoom, dBdt_flanks, ∇κ∇B_flanks, div_uB_flanks, u_bar_∇B_bar_flanks, 
                  u_prime∇B_prime_flanks, bin_center_saved, "Flanks (Zoom)", region_colors[2])
ax_flanks_zoom.set_ylim(0, 100)
# ax_flanks_zoom.set_xlim(-0.8,1.2)
ax_flanks_zoom.set_ylabel("Height above bottom [m]", fontsize=14)

plot_budget_region(ax_rest_zoom, dBdt_rest, ∇κ∇B_rest, div_uB_rest, u_bar_∇B_bar_rest, 
                  u_prime∇B_prime_rest, bin_center_saved, "Rest (Zoom)", region_colors[3])
ax_rest_zoom.set_ylim(0, 100)
# ax_rest_zoom.set_xlim(-0.8,1.2)
ax_rest_zoom.set_xlabel(L"[10^{-9} \, \text{m s}^{-3}]", fontsize=14)
ax_rest_zoom.legend(loc="upper right", fontsize=12, frameon=true, framealpha=0.93)
...
# Save figure
savefig(string("output/", simname, "/buoyancy_budget_regions_zoom_full_tᶠ=", tᶠ[1],"-",tᶠ[end], ".png"), 
        dpi=300, bbox_inches="tight")
println("Figure saved to output/", simname, "/buoyancy_budget_regions_zoom_full_tᶠ=", tᶠ[1],"-",tᶠ[end], ".png")
display(gcf())



# plot only the derivatives of the budget terms
close("all")
# Set font size globally
plt.rcParams["font.size"] = 14
...
# Define budget term colors to match your reference
budget_colors = [[150, 148, 255] / 255, [136, 194, 115] / 255, "orange", "red", "black","gray"]
budget_labels = [L"-\partial_{hab}\nabla\cdot(\overline{\mathbf{u}}\overline{B})", L" -\partial_{hab}\nabla\cdot\overline{\mathbf{u'}B'}", 
                L"-\partial_{hab}\overline{\nabla\cdot(\mathbf{u}B)}", L"-\partial_{hab}\nabla\cdot(\overline{\mathcal{B}})",
                L"\partial_{hab}d\overline{B}/dt",L"\partial_{hab}RHS"]

# Function to plot budget terms for each region
function plot_budget_region(ax, dBdt, ∇κ∇B, div_uB, u_bar_∇B_bar, u_prime∇B_prime, bin_center, title, region_color)
    # Plot budget terms
    ax.plot(.-u_bar_∇B_bar*1e10, bin_center, color=budget_colors[1], linewidth=2.5, label=budget_labels[1])
    ax.plot(.-u_prime∇B_prime*1e10, bin_center, color=budget_colors[2], linewidth=2.5, label=budget_labels[2])
    ax.plot(.-div_uB*1e10, bin_center, color=budget_colors[3], linewidth=2.5, linestyle="--", label=budget_labels[3])
    ax.plot(∇κ∇B*1e10, bin_center, color=budget_colors[4], linewidth=3, linestyle="-", label=budget_labels[4])
    ax.plot(dBdt*1e10, bin_center, color=budget_colors[5], linewidth=2.5, linestyle="-", label=budget_labels[5])
    ax.plot((∇κ∇B.-div_uB)*1e10, bin_center, color=budget_colors[6], linewidth=2.5, linestyle="--", label=budget_labels[6])
    # ax.plot(u_bar_∇B_bar*1e9, bin_center, color=budget_colors[1], linewidth=2.5, label=budget_labels[1])
    # ax.plot(u_prime∇B_prime*1e9, bin_center, color=budget_colors[2], linewidth=2.5, label=budget_labels[2])
    # ax.plot(div_uB*1e9, bin_center, color=budget_colors[3], linewidth=2.5, label=budget_labels[3])
    # ax.plot(∇κ∇B*1e9, bin_center, color=budget_colors[4], linewidth=3, linestyle=":", label=budget_labels[4])
    ...
    # ax.set_xticklabels([L"-10^{-9}", L"-10^{-10}", L"10^{-10}", L"10^{-9}"])
    # ax.set_xticklabels(["-3", "-2", "-1", "0", "1", "2", "3"])
    ax.set_xticklabels(["-1.5","-1", "-0.5", "0", "0.5", "1","1.5"])
...
bin_edge_deriv = bin_edge[2:end-1]
# Plot each region
plot_budget_region(ax_sill, dBdt_sill_deriv, ∇κ∇B_sill_deriv, div_uB_sill_deriv, u_bar_∇B_bar_sill_deriv, 
                  u_prime∇B_prime_sill_deriv, bin_edge_deriv, "Sill", region_colors[1])
# ax_sill.set_xlim(-0.5,0.5)
plot_budget_region(ax_flanks, dBdt_flanks_deriv, ∇κ∇B_flanks_deriv, div_uB_flanks_deriv, u_bar_∇B_bar_flanks_deriv, 
                  u_prime∇B_prime_flanks_deriv, bin_edge_deriv, "Flanks", region_colors[2])
# ax_flanks.set_xlim(-0.5,0.5)
ax_flanks.set_ylabel("Height above bottom [m]", fontsize=14)
plot_budget_region(ax_rest, dBdt_rest_deriv, ∇κ∇B_rest_deriv, div_uB_rest_deriv, u_bar_∇B_bar_rest_deriv, 
                  u_prime∇B_prime_rest_deriv, bin_edge_deriv, "Rest", region_colors[3])
# ax_rest.set_xlim(-0.5,0.5)
ax_rest.set_xlabel(L"[10^{-9} \, \text{m s}^{-3}]", fontsize=14)
# ax_rest.legend(loc="upper right", fontsize=12, frameon=true, framealpha=0.93)

# --- Zoomed-in budget plots (y=0:100) ---
plot_budget_region(ax_sill_zoom, dBdt_sill_deriv, ∇κ∇B_sill_deriv, div_uB_sill_deriv, u_bar_∇B_bar_sill_deriv, 
                  u_prime∇B_prime_sill_deriv, bin_edge_deriv, "Sill (Zoom)", region_colors[1])
ax_sill_zoom.set_ylim(0, 100)
# ax_sill_zoom.set_xlim(-0.5,0.5)
plot_budget_region(ax_flanks_zoom, dBdt_flanks_deriv, ∇κ∇B_flanks_deriv, div_uB_flanks_deriv, u_bar_∇B_bar_flanks_deriv, 
                  u_prime∇B_prime_flanks_deriv, bin_edge_deriv, "Flanks (Zoom)", region_colors[2])
ax_flanks_zoom.set_ylim(0, 100)
# ax_flanks_zoom.set_xlim(-0.5,0.5)
ax_flanks_zoom.set_ylabel("Height above bottom [m]", fontsize=14)

plot_budget_region(ax_rest_zoom, dBdt_rest_deriv, ∇κ∇B_rest_deriv, div_uB_rest_deriv, u_bar_∇B_bar_rest_deriv, 
                  u_prime∇B_prime_rest_deriv, bin_edge_deriv, "Rest (Zoom)", region_colors[3])
ax_rest_zoom.set_ylim(0, 100)
# ax_rest_zoom.set_xlim(-0.5,0.5)
ax_rest_zoom.set_xlabel(L"[10^{-10} \, \text{m s}^{-3}]", fontsize=14)
ax_rest_zoom.legend(loc="upper left", fontsize=12, frameon=true, framealpha=0.93)
...
# Save figure
savefig(string("output/", simname, "/buoyancy_budget_regions_zoom_full_tᶠ=", tᶠ[1],"-",tᶠ[end], "_deriv.png"), 
        dpi=300, bbox_inches="tight")
println("Figure saved to output/", simname, "/buoyancy_budget_regions_zoom_full_tᶠ=", tᶠ[1],"-",tᶠ[end], "_deriv.png")
display(gcf())
