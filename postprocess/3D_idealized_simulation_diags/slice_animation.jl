...
time_range = collect(1:length(t))
# time_range = 4801

# Function to load a single frame and mask topography
function load_frame(ds, idx)
    B_frame = ds["B"][:,1:1,:,idx]
    # Bz_frame = ds["Bz"][:,:,idx]
    # b_frame = ds["b"][:,:,idx]
    uhat_frame = ds["uhat"][:,1:1,:,idx]
    ε_frame = ds["ε"][:,1:1,:,idx]

    # set topography to NaN
    uhat_frame[uhat_frame.==0] .= NaN
    B_frame[ε_frame.==0] .= NaN
    ε_frame[ε_frame.==0] .= NaN
    Bz_frame = deriv(zC, B_frame)
    return uhat_frame[:,1,:], ε_frame[:,1,:], B_frame[:,1,:], Bz_frame[:,1,:]
end
