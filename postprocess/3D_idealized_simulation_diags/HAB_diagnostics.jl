...
function deriv(z, y)
    if ndims(y) == 2
        dydz = diff(y, dims=2) ./ reshape(diff(z), 1, length(z) - 1)
    elseif ndims(y) == 3
        dydz = diff(y, dims=3) ./ reshape(diff(z), 1, 1, length(z) - 1)
    elseif ndims(y) == 4
        dydz = diff(y, dims=4) ./ reshape(diff(z), 1, 1, 1, length(z) - 1)
    else
        throw(ArgumentError("Input y must be a 2D, 3D, or 4D array"))
    end
    return dydz
end
...
function rollmean_samesize(x, window)
    ...
    elseif ndims(x) == 2
        ...
        for j in 1:m
            ...
            j_end = min(m, j + half)
            result[:, j] = mean(x[:, j_start:j_end], dims=2)[:]
        end
    else
        throw(ArgumentError("Input x must be a 1D or 2D array"))
    end
    return result
end
...
fn1 = "output/3D_idealized_tilt_$(θ)/internal_tide_theta=$(θ)_Nx=2000_Nz=500_tᶠ=$(tᶠ)_yavg.nc"
ds = Dataset(fn1, "r")
...
Nx = length(xC);
t = ds["time"][:];
# ind_begin = findfirst(t / (2 * pi / 1.4e-4) .>= 20)
# time_range = length(t)-1:length(t)
time_range = 1:length(t)
...
title("Time-averaged div_uB from t=$(round(t[1]/(2*pi/1.4e-4))) to t=$(round(t[end]/(2*pi/1.4e-4)))")
grid(true)
show()
...
mask = [findfirst(div_uB[i, :] .> 0) for i in 1:Nx]
...
uhat_avg = nanmean(ds["uhat_avg"][:, :, time_range], dim=3)
what_avg = nanmean(ds["what_avg"][:, :, time_range], dim=3)
...
Bz_avg = deriv(zC, B_avg)
Bz_avg[div_uB_avg[:,1:end-1].==0] .= 0      # the grids are staggered, but this will effectively set the points inside and right above the immersed boudary to 0
Bz_center = (cat(zeros(Nx,1), Bz_avg, dims=2) .+ cat(Bz_avg, zeros(Nx,1), dims=2))/2 # average Bz at cell centers
...
for n in 1:length(time_range) ÷ 24
    ...
    uhat_temp = nanmean(ds["uhat_avg"][:, :, start:final], dim=3)
    what_temp = nanmean(ds["what_avg"][:, :, start:final], dim=3)
    ...
    B_temp[div_uB_temp.==0] .= NaN
    dB̄dx = mmderiv(xC[:], B_temp)
    dB̄dz = mmderiv(zC[:], B_temp[:, :]')'
    u_bar_∇B_bar_temp = u .* dB̄dx .+ w .* dB̄dz
    ...
    uhat_prime = ds["uhat_avg"][:, :, start:final] .- uhat_temp
    what_prime = ds["what_avg"][:, :, start:final] .- what_temp
    ...
    Bz_prime = similar(B_prime)
    for i in 1:(final - start + 1)
        Bz_prime[:, :, i] = mmderiv(zC[:], B_prime[:, :, i]')'
    end
...
tight_layout()
savefig(string("output/", simname, "/u_prime_dir_u_bar_and_kappa_gradient_buoyancy_pcolor.png"), dpi=150)
println("Saved figure to ", string("output/", simname, "/u_prime_dir_u_bar_and_kappa_gradient_buoyancy_pcolor.png"))
...
@time Bz_avg_hab = bin_stat_over_xy_overlapping(Bz_center, bin_start, bin_stop, bin_width, bin_stride, hab; stat="mean")
...
rm(string("output/", simname, "/TF_avg_analysis_$(θ)_$(tᶠ).nc"))  # remove existing file if any
ds_create = Dataset(string("output/", simname, "/TF_avg_analysis_$(θ)_$(tᶠ).nc"), "c")
...
tᶠ = 20.0
simname = "3D_idealized_tilt_$(θ)"
ds = Dataset(string("output/", simname, "/TF_avg_analysis_$(θ)_$(tᶠ).nc"), "r")
# load data
B_avg_hab = ds["B_avg"][:];
...
ax7.plot(-div_uB_avg_hab.+∇κ∇B_avg_hab, bin_center, label="RHS", color="black", linestyle="--")

RHS_smooth = rollmean_samesize(-div_uB_avg_hab.+∇κ∇B_avg_hab, 30)
ax7.plot(RHS_smooth, bin_center, label="RHS (smoothed)", linewidth=2)
...
savefig(string("output/", simname, "/TF_avg_analysis_", θ, "_", tᶠ, "_plots.png"), dpi=150)
println("Saved figure to ", string("output/", simname, "/TF_avg_analysis_", θ, "_", tᶠ, "_plots.png"))


## plot the buoyancy budget terms only for one case: 
fig = figure(figsize=(10, 5))
plot(dBdt_avg_hab, bin_center, label="dBdt_avg", linewidth=2, color="black")
plot(∇κ∇B_avg_hab, bin_center, label="∇κ∇B_avg", color="red")
plot(-div_uB_avg_hab, bin_center, label="-div_uB_avg", alpha=0.4, color="green")
# div_uB_smooth = rollmean_samesize(div_uB_avg_hab, 30)
# ax1.plot(-div_uB_smooth, bin_center, label="-div_uB_avg (smoothed)", linewidth=2, color="green")
set_ylabel("HAB (m)")
set_xlabel("Buoyancy budget (m/s³)")
set_title("Buoyancy budget (θ=0.008)")
grid(true)
legend(fontsize=8)
fig.tight_layout()
# Save the figure
savefig(string("output/", simname, "/TF_avg_analysis_buoyancy_budget_0.008.png"), dpi=150)
println("Saved figure to ", string("output/", simname, "/TF_avg_analysis_buoyancy_budget_0.008.png"))
...
## plot the time series of stratification
using NCDatasets
using NCDatasets
using Statistics
using NaNStatistics

fn1 = "output/3D_idealized_tilt_0.008/internal_tide_theta=0.008_Nx=2000_Nz=500_tᶠ=20.0_yavg.nc"
ds = Dataset(fn1, "r")
zC = ds["z_aac"][:];
Nx = length(ds["x_caa"][:]);
t = ds["time"][:];
...
bin_width = 20     # total window width
bin_stride = 5    # spacing between centers
bin_center = bin_start:bin_stride:bin_stop 
div_uB = ds["div_uB"][:, :, 1];  # just need the first time step to get the mask
mask = [findfirst(div_uB[ :] .> 0) for i in 1:Nx]
hab = repeat(zC, 1, Nx)' .- zC[mask]
Bz_avg_hab = zeros(length(bin_center), length(t))
for i in 1:length(t)
    B = ds["B"][:, :, i];
    div_uB_avg = nanmean(ds["div_uB"][:, :, :], dims=3)  # Ensure div_uB_avg is defined
    Bz_avg = deriv(zC, B)
    Bz_avg[div_uB_avg[:, 1:end-1] .== 0] .= 0  # The grids are staggered, but this will effectively set the points inside and right above the immersed boundary to 0
    Bz_center = (cat(zeros(Nx, 1), Bz_avg, dims=2) .+ cat(Bz_avg, zeros(Nx, 1), dims=2)) / 2  # Average Bz at cell centers
    Bz_center[Bz_center .== 0] .= NaN
    Bz_avg_hab[:,i] = bin_stat_over_xy_overlapping(Bz_center, bin_start, bin_stop, bin_width, bin_stride, hab; stat="mean")
    println("Processed time step ", i, " out of ", length(t))
end

# Plot the time series of Bz_avg_hab
Bz_avg_hab_smooth = rollmean_samesize(Bz_avg_hab, 30)
using PyPlot
figure(figsize=(10, 6))
pcolormesh(t./(2*pi/1.4e-4), bin_center, Bz_avg_hab_smooth, shading="auto", cmap="RdBu_r")
clim(8e-7,1.2e-6)
# contour(t./(2*pi/1.4e-4), bin_center, Bz_avg_hab, levels=1e-6:1e-6, colors="k", linewidths=1)
colorbar(label="Bz_avg (1/s²)")
xlabel("Time (tidal periods)")
ylabel("Height above bottom (m)")
title("Time series of terrain-following averaged Bz")
grid(true)
show()
savefig("output/3D_idealized_tilt_0.008/Bz_avg_hab_timeseries.png", dpi=150)
println("Saved figure to output/3D_idealized_tilt_0.008/Bz_avg_hab_timeseries.png")