...
# Load data for all specified time steps
...
# Load ALL data first, then unwrap (more robust approach)
@info "Loading all particle data files..."
all_raw_x = []
all_raw_y = []
all_raw_z = []
all_raw_b = []
...
for tᶠ in tᶠ_all
    ...
    push!(all_raw_x, data["x"][1:end,:])
    push!(all_raw_y, data["y"][1:end,:])
    push!(all_raw_z, data["z"][1:end,:])
    push!(all_raw_b, data["b"][1:end,:])
    push!(all_raw_time, data["time"][:])
end

# Concatenate all raw data
@info "Concatenating raw data..."
...
@info "Total particles: $n_particles, Total time steps: $n_times"

# Clear the raw data arrays to free memory
all_raw_x = nothing
all_raw_y = nothing
all_raw_z = nothing
all_raw_b = nothing
...
# Load grid data for buoyancy calculation (use first time step as reference)
...
# More memory-efficient approach: Process in chunks
@info "Processing data in chunks to manage memory..."

chunk_size = 1000  # Process 1000 particles at a time
n_chunks = ceil(Int, n_particles / chunk_size)

# Pre-allocate final output file
...
# Final wrapping check summary
if total_wrapping_issues == 0
    @info "✓ No wrapping issues found in final data!"
else
    @info "⚠ Found $total_wrapping_issues wrapping issues in final data"
end

# Add global attributes
ds_output.attrib["title"] = "Concatenated Particle Data"
...
@info "Concatenated data has been saved successfully!"
@info "Final data dimensions: $n_particles particles × $n_times time steps"
@info "Memory-efficient processing completed using $n_chunks chunks of size $chunk_size"