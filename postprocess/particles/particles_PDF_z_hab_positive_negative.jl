...
sum_z_neg = zeros(nx, ny)
count_z_neg = zeros(nx, ny)
...
# 4. Compute mean z in each bin (avoid division by zero)
mean_z_neg = fill(NaN, nx, ny)
for ix in 1:nx, iy in 1:ny
    if count_z_neg[ix, iy] > 60
        mean_z_neg[ix, iy] = sum_z_neg[ix, iy] / count_z_neg[ix, iy]
    end
end
...
# Prepare accumulator arrays
nx, ny = length(xedges)-1, length(yedges)-1
sum_z_pos = zeros(nx, ny)
count_z_pos = zeros(nx, ny)
...
# Compute mean z in each bin (avoid division by zero)
mean_z_pos = fill(NaN, nx, ny)
for ix in 1:nx, iy in 1:ny
    if count_z_pos[ix, iy] > 60  # filter out bins with too few particles
        mean_z_pos[ix, iy] = sum_z_pos[ix, iy] / count_z_pos[ix, iy]
    end
end


# interpolate the topography onto the particle grid
using Interpolations

# Assume topo_x and topo_y are 2D arrays (size: Nx_topo × Ny_topo)
# topo_z is also Nx_topo × Ny_topo

# Get 1D arrays for interpolation (if needed)
topo_x_1d = X_cart_interp_ext[:,1]  # x-coordinates
topo_y_1d = Y_cart_interp_ext[1,:]  # y-coordinates
topo_z = Z_cart_interp_ext  # z-coordinates

# Create interpolation object for topography
itp_topo = interpolate((topo_x_1d, topo_y_1d), topo_z, Gridded(Linear()))
...
# Prepare meshgrid for mean_z_neg grid
Xg, Yg = meshgrid(xcenters, ycenters)  # Xg, Yg are (nx, ny)

topo_x_min, topo_x_max = minimum(topo_x_1d), maximum(topo_x_1d)
topo_y_min, topo_y_max = minimum(topo_y_1d), maximum(topo_y_1d)

topo_on_particle_grid = [itp_topo(x, y) for x in xcenters*1e3, y in ycenters*1e3]  # (nx, ny)
mean_z_neg_hab = mean_z_neg .- topo_on_particle_grid
mean_z_pos_hab = mean_z_pos .- topo_on_particle_grid
...
# 2. Mean z (negative buoyancy change)
pcm = axs[2].pcolormesh(xcenters, ycenters, mean_z_neg_hab', cmap="GnBu_r", shading="auto", vmin=0, vmax=400)
...
# 3. Mean z (positive)
pcm = axs[3].pcolormesh(xcenters, ycenters, mean_z_pos_hab', cmap="GnBu_r", shading="auto", vmin=0, vmax=400)
...
cbar = fig.colorbar(pcm, ax=axs, extend="max", shrink=0.8)
cbar.ax.tick_params(labelsize=20)
cbar.set_label("HAB [m]", fontsize=20)
...
fig.savefig("output/$(simname)/combined_pdf_hab_panels_pos_neg_portion.png", dpi=100)
println("Figure saved to output/$(simname)/combined_pdf_hab_panels_pos_neg_portion.png")
close(fig)
...
count_z_upper = zeros(nx, ny)
sum_w_lower = zeros(nx, ny)
count_z_lower = zeros(nx, ny)
...
for ix in 1:nx, iy in 1:ny
    if count_z_upper[ix, iy] > 10
        mean_w_upper[ix, iy] = sum_w_upper[ix, iy] / count_z_upper[ix, iy]
    end
    if count_z_lower[ix, iy] > 10
        mean_w_lower[ix, iy] = sum_w_lower[ix, iy] / count_z_lower[ix, iy]
    end
end
